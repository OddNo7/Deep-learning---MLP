...
class MLP(object):
    ...
    top layer is a layer (defined here by a ``LogisticRegression``
    class).
    """
...
class myMLP(object):
    def __init__(self, rng, num_layers, input, n_in, n_hidden_array, n_out,activation):
        self.hiddenLayers=[]
        if num_layers==1:
                ...
                if i==0:
                    ...
                    self.hiddenLayers.append(
                        HiddenLayer(
                            ...
                            n_in=n_hidden_array[i-1],
                            n_out=n_hidden_array[i],
                            activation=activation
                        )
                    )
        if num_layers==1:
            ...
            n_in=n_hidden_array[0],
            n_out=n_out
        )
        else:
            self.logRegressionLayer = LogisticRegression(
                input=self.hiddenLayers[i].output,
                n_in=n_hidden_array[i],
                n_out=n_out
            )
        ...
        if num_layers==1:
            ...
            self.L2_sqr+=abs(self.hiddenLayers[0].W**2).sum()
        else:
            for i in range(num_layers):
                self.L1+=abs(self.hiddenLayers[i].W).sum()
                self.L2_sqr+=abs(self.hiddenLayers[i].W**2).sum()
...
def test_mlp(learning_rate=0.01, L1_reg=0.00, L2_reg=0.0001, n_epochs=500,
             batch_size=20, n_hidden=None, verbose=False, num_hidden_layers=1,activation=T.tanh):
    """
    ...
    if n_hidden==None:
        ...
        for i in range(num_hidden_layers):
            n_hidden_array.append(500)
    else:
        n_hidden_array=n_hidden
...
#    test_mlp(num_hidden_layers=2,activation=T.tanh)
#    test_mlp(num_hidden_layers=2,activation=T.nnet.softmax)
